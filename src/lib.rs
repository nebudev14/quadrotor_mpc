...
pub const QUADROTOR_MPC_N1: usize = 0;

/// Number of penalty constraints
pub const QUADROTOR_MPC_N2: usize = 0;
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(QUADROTOR_MPC_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, QUADROTOR_MPC_N1, QUADROTOR_MPC_N2)
}
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_quadrotor_mpc::get_w_cost();
...
pub fn solve(
    ...
    assert_eq!(p.len(), QUADROTOR_MPC_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), QUADROTOR_MPC_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    // Start by initialising the optimiser interface (e.g., set w=1)
    icasadi_quadrotor_mpc::init_quadrotor_mpc();
    ...
    if DO_PRECONDITIONING {
        ...
        icasadi_quadrotor_mpc::precondition(u, p);

        // Compute initial penalty
        icasadi_quadrotor_mpc::initial_penalty(u, p, & mut rho_init);
    }

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_quadrotor_mpc::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_quadrotor_mpc::grad(u, xi, p, grad);
        Ok(())
    };
