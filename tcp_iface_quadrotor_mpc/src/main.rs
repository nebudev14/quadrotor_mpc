///
/// Auto-generated TCP server for optimizer: quadrotor_mpc
///
...
fn execution_handler(
    ...
    match initial_guess {
        ...
        Some(u0) => {
            if u0.len() != QUADROTOR_MPC_NUM_DECISION_VARIABLES {
    ...
    if parameter.len() != QUADROTOR_MPC_NUM_PARAMETERS {
...
fn main() {
    let matches = App::new("OpEn TCP Server [quadrotor_mpc]")
        .version("0.0.0")
        .author("John Smith")
